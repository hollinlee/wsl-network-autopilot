...
The task runs every five minutes and starts the selected WSL distribution when needed. It launches WSL through the GUI-subsystem `wscript.exe` host with window style `0`, so periodic checks never create a console window. Remove it with:
